...
IMMUTABLE LANGUAGE sql AS $$
  SELECT length(h) <= 200
     AND h ~ '^scrypt\$[0-9]{1,9}\$[0-9]{1,4}\$[0-9]{1,4}\$[A-Za-z0-9_-]{8,}\$[A-Za-z0-9_-]{8,}$'
$$;
...
CREATE TABLE admin_account (
  ...
  password_hash  text        NOT NULL CHECK (is_password_hash(password_hash)),
  -- Base32, as every authenticator app expects. NULL: not enrolled.
  totp_secret    text        CHECK (totp_secret IS NULL OR totp_secret ~ '^[A-Z2-7]{16,64}$'),
  ...
  totp_pending_secret text  CHECK (totp_pending_secret IS NULL
...
-- admin can be shown how many are left.
CREATE TABLE admin_recovery_codes (
  id         uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
  code_hash  text        NOT NULL CHECK (is_password_hash(code_hash)),
  created_at timestamptz NOT NULL DEFAULT now(),
  used_at    timestamptz
);
...
-- what was revoked rather than forgetting it happened.
...
-- which is every link that existed before this migration. The redirect holds
-- this in its snapshot and verifies against it on the link's own domain; it
-- is never sent to a visitor.
ALTER TABLE links ADD COLUMN password_hash text
  CHECK (password_hash IS NULL OR is_password_hash(password_hash));
